...
module.exports = (projectId, opts) => {
	if (composeManager[projectId]) {
		return composeManager[projectId];
	} else {
		const tmp = new ComposeController(opts);
		composeManager[projectId] = tmp
		return tmp
	}
}
